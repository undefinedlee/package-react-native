...
function _parse(reactNativeDir, entries, platform, config, loader, callback){
	...
	parse(dirs, entries, platform, config, loader, function({fileHash, extensionFileHash}){
	...
	return function(){
		...
		this.plugin("loader-complete", function(info){
			if(info.packageJson.name === "react-native"){
				...
				_parse(info.path, entries, platform, info.config, info.loader, function(loadCache, extensionFileHash, packageJsons){
